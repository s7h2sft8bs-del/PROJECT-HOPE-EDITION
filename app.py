...
defaults = {
    'page': 'home', 'tier': 0, 'balance': 1000.0, 'starting_balance': 1000.0,
...
.glass-card {background: rgba(255, 255, 255, 0.03); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 24px; padding: 28px; margin: 12px 0; box-shadow: 0 8px 32px rgba(0, 0, 0, 0.4), inset 0 1px 0 rgba(255, 255, 255, 0.05);}
.glass-card-sm {background: rgba(255, 255, 255, 0.02); backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.06); border-radius: 16px; padding: 20px; margin: 8px 0;}
...
.logo-text {font-size: 2.8em; font-weight: 900; letter-spacing: -1px; background: linear-gradient(135deg, #00FFA3 0%, #00E5FF 50%, #FFD700 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;}
.hero-section {text-align: center; padding: 60px 40px; background: linear-gradient(145deg, rgba(0, 255, 163, 0.08) 0%, rgba(0, 229, 255, 0.05) 50%, rgba(255, 215, 0, 0.03) 100%); backdrop-filter: blur(20px); border-radius: 32px; margin: 20px 0 30px 0; border: 1px solid rgba(255, 255, 255, 0.1); box-shadow: 0 20px 60px rgba(0, 0, 0, 0.5);}
.hero-title {font-size: 3.5em; font-weight: 900; background: linear-gradient(135deg, #00FFA3 0%, #00E5FF 40%, #FFD700 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin: 0 0 20px 0;}
...
.shield-container {background: linear-gradient(145deg, rgba(0, 255, 163, 0.12), rgba(0, 200, 100, 0.05)); border: 2px solid rgba(0, 255, 163, 0.35); border-radius: 20px; padding: 24px 32px; text-align: center; margin: 20px 0;}
...
.stat-card {background: rgba(255, 255, 255, 0.03); backdrop-filter: blur(16px); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 20px; padding: 24px; text-align: center;}
...
.tier-card {background: rgba(255, 255, 255, 0.02); backdrop-filter: blur(20px); border-radius: 28px; padding: 32px 20px; text-align: center; border: 1px solid rgba(255, 255, 255, 0.08); min-height: 380px; position: relative;}
.tier-card::before {content: ''; position: absolute; top: 0; left: 0; right: 0; height: 4px; border-radius: 28px 28px 0 0;}
...
.tier-master {box-shadow: 0 0 50px rgba(255, 215, 0, 0.15); border: 1px solid rgba(255, 215, 0, 0.2);}
...
.popular-badge {background: linear-gradient(135deg, #FFD700, #FFA500); color: black; font-size: 0.7em; font-weight: 700; padding: 4px 12px; border-radius: 20px; text-transform: uppercase; letter-spacing: 1px; display: inline-block; margin-bottom: 8px;}
.stock-card {background: rgba(255, 255, 255, 0.03); backdrop-filter: blur(16px); border-radius: 20px; padding: 24px; margin: 12px 0; border: 1px solid rgba(255, 255, 255, 0.08);}
...
def get_market_status():
    ...
    if now.weekday() >= 5:
        return 'closed', current_time, "WEEKEND"
    if now < market_open:
        diff = market_open - now
        h, m, s = diff.seconds // 3600, (diff.seconds % 3600) // 60, diff.seconds % 60
        return 'pre', current_time, f"PRE-MARKET {h:02d}:{m:02d}:{s:02d}"
    if now < market_close:
        diff = market_close - now
        h, m, s = diff.seconds // 3600, (diff.seconds % 3600) // 60, diff.seconds % 60
        return 'open', current_time, f"MARKET OPEN {h:02d}:{m:02d}:{s:02d}"
    return 'closed', current_time, "AFTER HOURS"

def generate_stock_data(stock):
    symbol, base = stock['symbol'], stock['base_price']
    if symbol not in st.session_state.stock_data:
        ...
        st.session_state.stock_data[symbol] = {'prices': prices, 'volumes': [random.randint(1000000, 10000000) for _ in range(100)]}
...
def analyze_stock(stock):
    ...
    return {'symbol': stock['symbol'], 'name': stock['name'], 'price': price, 'change': round(price - prices[-2] if len(prices) > 1 else 0, 2), 'score': score, 'signal': signal, 'signals': signals, 'option_cost': option_cost, 'stop_loss': round(option_cost * 0.75, 2), 'take_profit': round(option_cost * 1.30, 2)}
...
def generate_share_text():
    ...
    return f"PROJECT HOPE Results\n\nToday: ${st.session_state.daily_pnl:+,.2f}\nWin Rate: {wr:.0f}%\nW/L: {st.session_state.wins}/{st.session_state.losses}\n\n#ProjectHope #Options"
...
def render_home():
    ...
    st.markdown(f'<div class="clock-container clock-{status}"><p class="clock-time">{current_time}</p><p class="clock-status">{countdown}</p></div>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown("### Why Options Beat Stocks")
    c1, c2 = st.columns(2)
    with c1: st.markdown('<div class="glass-card" style="border-left: 4px solid #FF4B4B;"><h3 style="color: #FF4B4B; margin: 0 0 12px 0;">Old Way (Stocks)</h3><p style="color: white; margin: 0; font-size: 1.1em;">$200 invested, 1% gain = <strong style="color: #FF4B4B;">$2 profit</strong></p></div>', unsafe_allow_html=True)
    with c2: st.markdown('<div class="glass-card" style="border-left: 4px solid #00FFA3;"><h3 style="color: #00FFA3; margin: 0 0 12px 0;">New Way (Options)</h3><p style="color: white; margin: 0; font-size: 1.1em;">$200 invested, 1% gain = <strong style="color: #00FFA3;">$100+ profit</strong></p></div>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    with st.expander("Read My Full Story"):
        st.markdown(f'<p style="color: #c0c0c0; line-height: 1.8; white-space: pre-line;">{STEPHEN_BIO}</p>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    with c2:
        ...
        if code:
            if code.upper() in codes:
                ...
                st.success(f"Access Granted: {TIERS[st.session_state.tier]['name']} Tier")
    ...
    st.markdown(f'''<div class="legal-footer"><p style="color: #808495; font-size: 1em; margin: 0 0 10px 0;">Built with love by {STEPHEN_NAME} | {STEPHEN_EMAIL}</p><p style="color: #666; font-size: 0.85em; margin: 0 0 15px 0;">This is an educational tool only. Not financial advice.</p></div>''', unsafe_allow_html=True)
    with st.expander("Read Full Legal Disclaimer"):
        st.markdown(f'<p style="color: #808495; line-height: 1.8; white-space: pre-line; font-size: 0.9em;">{LEGAL_DISCLAIMER}</p>', unsafe_allow_html=True)
...
def render_trade():
    ...
    with c2:
        ...
        st.markdown(f'<div class="clock-container clock-{status}" style="padding:10px 16px;"><span style="font-size:0.9em;font-weight:600;">{countdown}</span></div>', unsafe_allow_html=True)
    with c3: st.markdown('<div style="text-align:right;padding:10px;"><span style="color:#FFD700;font-weight:600;background:rgba(255,215,0,0.1);padding:6px 14px;border-radius:8px;">PAPER MODE</span></div>', unsafe_allow_html=True)
    c1, c2, c3, c4 = st.columns(4)
    with c1:
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1: st.metric("Equity", f"${equity:,.2f}")
    with c2: st.metric("Cash", f"${st.session_state.balance:,.2f}")
    with c3: st.metric("Today P/L", f"${st.session_state.daily_pnl:+,.2f}")
    with c4:
        total = st.session_state.wins + st.session_state.losses
        wr = (st.session_state.wins / total * 100) if total > 0 else 0
        st.metric("Win Rate", f"{wr:.0f}%")
    if tier['autopilot'] == 'always': st.markdown('<div class="autopilot-on"><span style="color:#00FFA3;font-weight:700;font-size:1.1em;">AUTOPILOT: ALWAYS ON</span><br><span style="color:#808495;font-size:0.9em;">Starter tier runs fully automatic</span></div>', unsafe_allow_html=True)
    else:
        c1, c2 = st.columns([4, 1])
        with c1:
            if st.session_state.autopilot: st.markdown('<div class="autopilot-on"><span style="color:#00FFA3;font-weight:700;font-size:1.1em;">AUTOPILOT: ON</span><br><span style="color:#808495;font-size:0.9em;">Auto-executes signals, manages stops & targets</span></div>', unsafe_allow_html=True)
            else: st.markdown('<div class="autopilot-off"><span style="color:#808495;font-weight:700;font-size:1.1em;">AUTOPILOT: OFF</span><br><span style="color:#808495;font-size:0.9em;">Manual trading mode</span></div>', unsafe_allow_html=True)
        with c2:
            if st.button("Toggle", use_container_width=True, key="toggle_ap"): st.session_state.autopilot = not st.session_state.autopilot; st.rerun()
    st.markdown(f'<div class="shield-container"><p class="shield-title">5-LAYER PROTECTION ACTIVE</p><p class="shield-subtitle">Stop -25% | Profit +30% | Daily -15% | 5% Per Trade | Max {tier["max_trades"]} Positions</p></div>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    col1, col2 = st.columns([2, 1])
    with col1:
        st.markdown("### Stock Scanner")
        st.markdown(f'<p style="color:#808495;">Top {tier["stocks_shown"]} opportunities</p>', unsafe_allow_html=True)
        ...
        for stock in stocks:
            ...
            st.markdown(f'<div class="stock-card {card_class}"><div style="display:flex;justify-content:space-between;align-items:center;"><div><h3 style="color:white;margin:0;font-size:1.4em;">{stock["symbol"]}</h3><p style="color:#808495;margin:4px 0 0 0;">{stock["name"]}</p></div><div style="text-align:right;"><h3 style="color:#00E5FF;margin:0;font-size:1.4em;">${stock["price"]:.2f}</h3><p style="color:{chg_color};margin:4px 0 0 0;font-weight:600;">{stock["change"]:+.2f}</p></div></div><div class="score-bar"><div class="score-fill {bar_class}" style="width:{bar_pct}%;"></div></div><div style="display:flex;justify-content:space-between;align-items:center;"><span style="color:{sig_color};font-weight:700;font-size:1.1em;">{stock["signal"]} ({stock["score"]}/8)</span><span style="color:#808495;">~${int(stock["option_cost"])}/contract</span></div></div>', unsafe_allow_html=True)
            ind_html = "".join([f'<span class="indicator ind-{s[1]}">{n}</span>' for n, s in stock['signals'].items()])
            st.markdown(f'<div style="margin:-5px 0 15px 0;">{ind_html}</div>', unsafe_allow_html=True)
            if stock['signal'] in ['STRONG BUY', 'BUY', 'STRONG SELL', 'SELL'] and tier['autopilot'] != 'always' and len(st.session_state.positions) < tier['max_trades']:
                ...
                if st.button(f"BUY {direction} - ${int(stock['option_cost'] * 100)}", key=f"buy_{stock['symbol']}", use_container_width=True):
                    if execute_buy(stock, direction): st.success("Position opened!"); st.balloons(); st.rerun()
    with col2:
        st.markdown(f"### Positions ({len(st.session_state.positions)}/{tier['max_trades']})")
        if st.session_state.positions:
            for i, pos in enumerate(st.session_state.positions):
                pc = "#00FFA3" if pos['pnl'] >= 0 else "#FF4B4B"
                st.markdown(f'<div class="position-card" style="border-left:3px solid {pc};"><div style="display:flex;justify-content:space-between;"><div><h4 style="color:white;margin:0;">{pos["symbol"]}</h4><p style="color:#808495;font-size:0.85em;margin:4px 0 0 0;">{pos["direction"]} @ ${pos["entry"]:.2f}</p></div><div style="text-align:right;"><h4 style="color:{pc};margin:0;">${pos["pnl"]:+.2f}</h4></div></div></div>', unsafe_allow_html=True)
                if tier['autopilot'] != 'always' and not st.session_state.autopilot:
                    if st.button("Close", key=f"close_{i}", use_container_width=True): execute_sell(i); st.rerun()
...
def render_history():
    ...
    st.markdown(f'<div class="share-card"><div style="margin-bottom:20px;"><span style="font-size:2.5em;">&#127793;</span><span style="font-size:1.8em;font-weight:800;background:linear-gradient(135deg,#00FFA3,#FFD700);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-left:10px;">PROJECT HOPE</span></div><p style="color:#808495;margin:0 0 20px 0;">{datetime.now().strftime("%B %d, %Y")}</p><h1 style="color:{pnl_c};font-size:3em;margin:0;">${st.session_state.daily_pnl:+,.2f}</h1><p style="color:{pnl_c};font-size:1.2em;margin:5px 0 25px 0;">({daily_ret:+.1f}%)</p><div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;margin-bottom:20px;"><div><p style="color:#808495;margin:0;">Trades</p><p style="color:white;font-size:1.5em;margin:5px 0 0 0;">{total}</p></div><div><p style="color:#808495;margin:0;">Win Rate</p><p style="color:#FFD700;font-size:1.5em;margin:5px 0 0 0;">{wr:.0f}%</p></div><div><p style="color:#808495;margin:0;">W/L</p><p style="color:white;font-size:1.5em;margin:5px 0 0 0;">{st.session_state.wins}/{st.session_state.losses}</p></div></div></div>', unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    ...
    if st.session_state.trades:
        for t in reversed(st.session_state.trades[-10:]):
            ...
            st.markdown(f'<div class="glass-card-sm" style="border-left:3px solid {c};"><div style="display:flex;justify-content:space-between;"><div><p style="color:#808495;font-size:0.85em;margin:0;">{t.get("date", "")} {t["time"]}</p><h4 style="color:white;margin:8px 0 0 0;">{t["symbol"]} {t.get("direction", "")}</h4></div><h3 style="color:{c};margin:0;">${t["pnl"]:+.2f}</h3></div></div>', unsafe_allow_html=True)
...
def render_learn():
    ...
    for name, desc, color in [("RSI", "Oversold <30 = Buy, Overbought >70 = Sell", "#00FFA3"), ("EMA", "9/21 crossover signals trend changes", "#00E5FF"), ("VWAP", "Institutional buying/selling levels", "#FFD700"), ("S/R", "Support & Resistance key levels", "#A855F7"), ("VOL", "Volume spikes confirm real moves", "#FF6B6B")]:
        st.markdown(f'<div class="glass-card-sm" style="border-left:4px solid {color};"><h4 style="color:{color};margin:0 0 8px 0;">{name}</h4><p style="color:#c0c0c0;margin:0;">{desc}</p></div>', unsafe_allow_html=True)
